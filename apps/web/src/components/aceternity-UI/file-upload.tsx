...
export const FileUpload = ({
  ...
  useEffect(() => {
    if (currentFiles < filesRef.current.length) {
      const updatedFiles = filesRef.current.slice(0, currentFiles)
      filesRef.current = updatedFiles
      setFiles(updatedFiles)

      if (currentFiles === 0 && fileInputRef.current) {
              ...
              <div className="relative mx-auto mt-10 w-full max-w-xl">
                {files.map((file, idx) => (
                  <motion.div
                    ...
                    className="relative z-40 mx-auto mt-4 flex w-full flex-col items-start justify-start overflow-hidden rounded-md bg-white p-4 dark:bg-neutral-900 md:h-24"
                  >
                    <div className="flex w-full items-center justify-between gap-4">
                      <motion.p className="max-w-xs truncate text-base text-neutral-700 dark:text-neutral-300">
                        {file.name}
                      </motion.p>
                      <motion.p className="w-fit flex-shrink-0 rounded-lg px-2 py-1 text-sm text-neutral-600 shadow-input dark:bg-neutral-800 dark:text-white">
                    ...
                    <div className="mt-2 flex w-full flex-col items-start justify-between text-sm text-neutral-600 dark:text-neutral-400 md:flex-row md:items-center">
                      <motion.p className="rounded-md bg-gray-100 px-1 py-0.5 dark:bg-neutral-800">
                        {file.type}
                      </motion.p>
                      <motion.p>
                        modified{' '}
